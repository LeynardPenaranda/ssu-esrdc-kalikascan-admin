...
import { uploadToCloudinaryAdminProfile } from "@/src/cloudinary/uploadToCloudinaryAdminProfile";
import { useToast } from "@/src/hooks/useToast";
import UpdateAdminNameModal from "@/src/components/modals/UpdateAdminNameModal";
...
export default function AdminProfilePage() {
  ...
  const handleChange: UploadProps["onChange"] = ({ fileList: newList }) => {
    setFileList(newList.slice(-1)); // limit to 1
  };
  ...
  async function onConfirmUpload() {
    ...
    try {
      const uploaded = await uploadToCloudinaryAdminProfile(selected, user.uid);
      const token = await user.getIdToken(true);

      const res = await fetch("/api/admin/profile/update-photo", {
        method: "POST",
        headers: {
